...
python3 -u grpo.py \
  --use_prefill \
  --model_name /root/autodl-tmp/Qwen3-8B-L6200-review_stage2 \
  --data_path /root/autodl-tmp/PruneRL/datasets/thinkprune_mini_20.json \
  --current_limit 4800 \
  --gen_max_new_tokens 5300 \
  --num_generations 4 \
  ...
  --logging_steps 1 \
  --save_steps 4 \
  --output_dir ./grpo_qwen3_L4800_stage3_review \
  --debug_group_rewards_path /root/autodl-tmp/PruneRL/code/grpo_qwen_rewards_stage3.jsonl

GLM-Z1
python3 -u grpo.py \
  --use_prefill \
  --prompt_style glmz1 \
  --model_name /root/autodl-tmp/GLM-Z1-9B-L5300-review_stage2 \
  --data_path /root/autodl-tmp/PruneRL/datasets/thinkprune_mini_20.json \
  --current_limit 4200 --gen_max_new_tokens 4700 \
  --num_generations 4 \
  --per_device_train_batch_size 1 --gradient_accumulation_steps 4 \
  --temperature 0.6 --top_p 0.9 --repetition_penalty 1.2 \
  --num_iterations 1 \
  --max_steps 40 --logging_steps 1 --save_steps 4 \
  --output_dir /root/autodl-tmp/PruneRL/code/grpo_glmz1_L4200_stage3_review \
  --debug_group_rewards_path /root/autodl-tmp/PruneRL/code/grpo_glmz1_rewards_stage3.jsonl
...
python3 -u grpo.py \
  ...
  --append_r1_eos \
  --model_name /root/autodl-tmp/R1-Distill-Qwen-7B-L5100-review_stage2 \
  --data_path /root/autodl-tmp/PruneRL/datasets/thinkprune_mini_20.json \
  --current_limit 3700 --gen_max_new_tokens 4100 \
  --num_generations 4 \
  --per_device_train_batch_size 1 --gradient_accumulation_steps 4 \
  --temperature 0.6 --top_p 0.9 --repetition_penalty 1.2 \
  --num_iterations 1 \
  --max_steps 40 --logging_steps 1 --save_steps 4 \
  --output_dir /root/autodl-tmp/PruneRL/code/grpo_r1distill_L3700_stage3_review \
  --debug_group_rewards_path /root/autodl-tmp/PruneRL/code/grpo_r1distill_rewards_stage3.jsonl
'''
...
def think_prune_reward_fn_builder(
    ...
    *,
    ...
    def structure_flags(text: str):
        t = text.lower()
        has_kp = "**key points**" in t
        has_sol = "**solution**" in t
        has_final = ("**final answer**" in t) or ("\\boxed" in t) or ("答案" in t)
        has_think_close = "</think>" in t
        return has_kp, has_sol, has_final, has_think_close
    ...
    def think_prune_reward_fn(completions, ground_truth, problem, **kwargs):
        ...
        for idx, (completion, gt, prob) in enumerate(zip(completions, ground_truth, problem)):
            ...
            if debug_group_rewards_path and (
                ...
                has_kp, has_sol, has_final, has_think_close = structure_flags(clipped_text)
                ...
                debug_rows.append(
                    {
                        ...
                        "token_len": token_len,
                        "len_ratio": round(token_len / float(current_limit), 6),
...
def main():
    ...
    trainer.train()
    # trainer.train(resume_from_checkpoint="/root/autodl-tmp/PruneRL/code/grpo_qwen3_L6200_stage2_review/checkpoint-32")
