...
python3 -u grpo_3.py \
  --use_prefill \
  --model_name /root/autodl-tmp/model/wangsijin/Qwen3-8B-7cat \
  --data_path /root/autodl-tmp/PruneRL/datasets/thinkprune_mini_20.json \
  --current_limit 6800 \
  --gen_max_new_tokens 7800 \
  --num_generations 4 \
  ...
  --save_steps 4 \
  --early_stop \
  --debug_group_rewards_path ./grpo_3_qwen_rewards_stage1_innovation.jsonl \
  --debug_group_rewards_max_calls 0 \
  --output_dir ./grpo_3_qwen3_L6800_stage1_innovation

GLM-Z1
python3 -u grpo_3_stage1.py \
  --use_prefill \
  --prompt_style glmz1 \
  --model_name /root/autodl-tmp/model/wangsijin/GLM-Z1-9B-7cat \
  --data_path /root/autodl-tmp/PruneRL/datasets/thinkprune_mini_20.json \
  --current_limit 5900 --gen_max_new_tokens 6500 \
  --num_generations 4 \
  ...
  --early_stop \
  --output_dir /root/autodl-tmp/PruneRL/code/grpo_3_glmz1_L5900_stage1_innovation \
  --debug_group_rewards_path /root/autodl-tmp/PruneRL/code/grpo_3_glmz1_rewards_stage1_innovation.jsonl
...
python3 -u grpo_3_stage1.py \
  ...
  --append_r1_eos \
  --model_name /root/autodl-tmp/model/wangsijin/R1-Distill-Qwen-7B-7cat \
  --data_path /root/autodl-tmp/PruneRL/datasets/thinkprune_mini_20.json \
  --current_limit 5700 --gen_max_new_tokens 6300 \
  --num_generations 4 \
  --per_device_train_batch_size 1 --gradient_accumulation_steps 4 \
  --temperature 0.6 --top_p 0.9 --repetition_penalty 1.2 \
  --num_iterations 1 \
  --max_steps 40 --logging_steps 1 --save_steps 4 \
  --early_stop \
  --output_dir /root/autodl-tmp/PruneRL/code/grpo_3_r1distill_L5700_stage1_innovation \
  --debug_group_rewards_path /root/autodl-tmp/PruneRL/code/grpo_3_r1distill_rewards_stage1_innovation.jsonl
'''
    ...
    *,
    ...
    def think_prune_reward_fn(completions, ground_truth, problem, **kwargs):
        ...
        for idx, (completion, gt, prob) in enumerate(zip(completions, ground_truth, problem)):
            ...
            if ok:
                # 【做对分支】：基础分 + 格式分 + 长度压缩加成
                # len_ratio越小，说明字数越短，越能吃满这 0.2 的额外加成
                length_bonus = 0.2 * (1.0 - len_ratio)
                reward = 1.0 + (0.5 * x3) + length_bonus
            else:
                # 1. 引入物理截断检查 (Clipped Check)
                # 只要撞墙 (len_ratio >= 1.0)，基础分直接给 -0.5，格式分全部抹杀
                if len_ratio >= 1.0:
                    base_fail_score = -0.5
                else:
                    # 如果没撞墙但错了，格式合格给 0.05 引导，不合格给 -0.1
                    base_fail_score = 0.05 if (x3 >= 0.55) else -0.1
                
                # 2. 强化长度惩罚系数：从 0.5 提升至 1.5
                length_penalty = 1.5 * len_ratio
                
                # 最终 Reward (撞墙状态)：-0.5 - 1.5 = -2.0 (这是第二版的最低分)
